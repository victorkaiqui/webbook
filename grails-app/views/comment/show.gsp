	...
	<body>
		...
		<div id="show-comment" class="content scaffold-show" role="main">
			...
			<ol class="property-list comment">
				...
				<g:if test="${commentInstance?.bookmark}">
				<li class="fieldcontain">
					<span id="bookmark-label" class="property-label"><g:message code="comment.bookmark.label" default="Bookmark" /></span>
					
						<span class="property-value" aria-labelledby="bookmark-label"><g:link controller="bookmark" action="show" id="${commentInstance?.bookmark?.id}">${commentInstance?.bookmark?.encodeAsHTML()}</g:link></span>
